// spec: test-plan-console-logs-commands.md
// seed: tests/seed.spec.ts

import { test, expect } from '@playwright/test';

test.describe('Execute "test" Command', () => {
  test('Scenario 3: Execute "test" Command', async ({ page }) => {
    // Navigate to authentication URL to authenticate via token
    await page.goto('https://javascriptextensions.dev.localhost:17253/login?t=99c3f542e0e4e1916be5148b2d2e1199');
    
    // Wait for authentication to complete
    await new Promise(f => setTimeout(f, 3 * 1000));
    
    // Navigate to console logs page for JavaScriptAppWithCommands resource
    await page.goto('https://javascriptextensions.dev.localhost:17253/consolelogs/resource/JavaScriptAppWithCommands');
    
    // Wait for page to fully load
    await new Promise(f => setTimeout(f, 3 * 1000));
    
    // Click on "Resource actions" button to open menu
    await page.getByRole('button', { name: 'Resource actions' }).click();
    
    // Hover over the "Commands" menuitem to expand submenu
    await page.getByRole('menuitem', { name: 'Commands' }).hover();
    
    // Click on the "test" menuitem in the Commands submenu
    await page.getByRole('menuitem', { name: 'test' }).click();
    
    // Verify "Executing command 'test'." log entry is visible
    const executing = await page.getByText('Executing command \'test\'.').all();
    expect(executing.length).toBeGreaterThan(0);
    
    
    // Verify command execution path is logged
    const commandPath = await page.getByText('Exec C:\\Program Files\\nodejs\\npm.cmd run test in folder:').all();
    expect(commandPath.length).toBeGreaterThan(0);
    
    // Verify "Successfully executed command 'test'." log entry is visible
    const successMessage = await page.getByText('Successfully executed command \'test\'.').all();
    expect(successMessage.length).toBeGreaterThan(0);
  });
});
